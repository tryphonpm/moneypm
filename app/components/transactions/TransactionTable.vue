...
function toggleAll() {
  selected.value = allSelected.value
    ? new Set()
    : new Set(props.transactions.map((t) => t._id))
}
...
// ── Tri (côté client sur la page courante) ─────────────────
...
const sorted = computed(() =>
  [...props.transactions].sort((a, b) => {
    ...
    switch (sortKey.value) {
      ...
      case 'beneficiaire':cmp = a.beneficiaire.localeCompare(b.beneficiaire); break
  ...
  <div class="space-y-3">
    ...
    <div class="flex items-center justify-between px-1">
      <p class="text-sm text-gray-500 dark:text-gray-400">
        <span class="font-medium text-gray-900 dark:text-gray-100">{{ total ?? transactions.length }}</span>
        écriture{{ (total ?? transactions.length) > 1 ? 's' : '' }}
        <template v-if="selected.size > 0">
          · <span class="text-primary-400 font-medium">{{ selected.size }} sélectionnée{{ selected.size > 1 ? 's' : '' }}</span>
        </template>
      </p>
      ...
      <table class="data-table">
        <thead>
          <tr>
            ...
            <th class="sortable w-28" @click="toggleSort('date')">
              Date <span class="text-gray-500 ml-1">{{ sortIcon('date') }}</span>
            </th>
            <th class="sortable w-36" @click="toggleSort('type')">
              Type <span class="text-gray-500 ml-1">{{ sortIcon('type') }}</span>
            </th>
            <th class="sortable" @click="toggleSort('beneficiaire')">
              Bénéficiaire <span class="text-gray-500 ml-1">{{ sortIcon('beneficiaire') }}</span>
            </th>
            <th class="sortable text-right w-28" @click="toggleSort('montant')">
              Retrait <span class="text-gray-500 ml-1">{{ sortIcon('montant') }}</span>
            </th>
        ...
        <tbody>
          ...
          <template v-if="loading">
            <tr v-for="i in limit ?? 25" :key="i">
              <td colspan="10" class="py-3">
                <div class="h-4 bg-gray-800 rounded animate-pulse mx-3" />
              </td>
            </tr>
          </template>
          ...
          <tr v-else-if="sorted.length === 0">
            <td colspan="10" class="py-12 text-center text-gray-500 text-sm">
          ...
          >
            ...
            <td class="text-gray-400 text-xs font-mono">{{ fmtDate(tx.date) }}</td>
            <td>
              <span class="badge bg-gray-700/60 text-gray-300 text-xs">{{ tx.type }}</span>
            </td>
            <td class="max-w-[240px] truncate" :title="tx.beneficiaire">
              {{ tx.beneficiaire || '—' }}
            </td>
            <td class="text-right">
              <span v-if="tx.montant < 0" class="amount-debit">{{ fmtAmount(tx.montant) }} €</span>
            </td>
            <td class="text-right">
              <span v-if="tx.montant >= 0" class="amount-credit">{{ fmtAmount(tx.montant) }} €</span>
            </td>
            <td>
            ...
            <td class="text-center">
              <button class="btn-icon" title="Éditer" @click="emit('edit', tx)">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
